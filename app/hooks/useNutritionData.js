...
const useNutritionData = () => {
    ...
    const analyzeNutrition = useCallback((nutritionData) => {
        ...
        const isSafe = sugarPer100g <= THRESHOLDS.SUGAR &&
        ...
        if (sugarPer100g > THRESHOLDS.SUGAR) {
            analysis.details.warnings.push(`High sugar content (${sugarPer100g.toFixed(1)}g per 100g)`);
        }
        if (carbsPer100g > THRESHOLDS.CARBS) {
            analysis.details.warnings.push(`High carbohydrate content (${carbsPer100g.toFixed(1)}g per 100g)`);
        }
        if (fatPer100g > THRESHOLDS.FAT) {
            analysis.details.warnings.push(`High fat content (${fatPer100g.toFixed(1)}g per 100g)`);
        }
        if (saturatedFatPer100g > THRESHOLDS.SATURATED_FAT) {
            analysis.details.warnings.push(`High saturated fat content (${saturatedFatPer100g.toFixed(1)}g per 100g)`);
        }
        if (sodiumPer100g > THRESHOLDS.SODIUM) {
            analysis.details.warnings.push(`High sodium content (${sodiumPer100g.toFixed(1)}mg per 100g)`);
        }
        if (cholesterolPer100g > THRESHOLDS.CHOLESTEROL) {
            analysis.details.warnings.push(`High cholesterol content (${cholesterolPer100g.toFixed(1)}mg per 100g)`);
        }
    ...
    const validateQuery = useCallback((query) => {
        ...
        if (hasQuantity && !/^\d+(\.\d+)?\s*[a-zA-Z]+/.test(query)) {
            throw new Error('Invalid quantity format. Example: "100g chicken" or "1 apple"');
        }
    ...
    const retryFetch = async (query, retryCount = 0) => {
        try {
            ...
            if (response.data.calories === 0 && !query.toLowerCase().includes('water')) {
    ...
    const fetchNutritionData = async (rawQuery) => {
        try {
            ...
            const errorMessage = err.response?.data?.message || err.message || 'Failed to fetch nutrition data';
